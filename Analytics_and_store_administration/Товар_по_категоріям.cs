        ...
        private void DeleteButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(dbHelper.GetConnection().ConnectionString))
                {
                    ...
                    using (SqlCommand checkZmistRealizatsiyiRecordsCmd = new SqlCommand("SELECT COUNT(*) FROM Зміст_реалізації WHERE ID_товару IN (SELECT ID_товару FROM Товар WHERE ID_категорії = @KategoryId)", connection))
                    {
                        checkZmistRealizatsiyiRecordsCmd.Parameters.AddWithValue("@KategoryId", kategoryId);
                    ...
                    {
                        using (SqlCommand checkRealizatsiyaRecordsCmd = new SqlCommand("SELECT COUNT(*) FROM Реалізація WHERE ID_покупки IN (SELECT ID_покупки FROM Зміст_реалізації WHERE ID_товару IN (SELECT ID_товару FROM Товар WHERE ID_категорії = @KategoryId))", connection))
                        {
                            checkRealizatsiyaRecordsCmd.Parameters.AddWithValue("@KategoryId", kategoryId);
                    ...
                    if (!hasRelatedRecords)
                    {
                        using (SqlCommand checkTovarRecordsCmd = new SqlCommand("SELECT COUNT(*) FROM Товар WHERE ID_категорії = @KategoryId", connection))
                        {
                            checkTovarRecordsCmd.Parameters.AddWithValue("@KategoryId", kategoryId);
                    ...
                    if (result == DialogResult.Yes)
                    {
                        ...
                        if (hasRelatedRecords)
                        {
                            string deleteZmistRealizatsiyiQuery = "DELETE FROM Зміст_реалізації WHERE ID_товару IN (SELECT ID_товару FROM Товар WHERE ID_категорії = @KategoryId)";
                            using (SqlCommand deleteZmistRealizatsiyiCmd = new SqlCommand(deleteZmistRealizatsiyiQuery, connection))
                            {
                                deleteZmistRealizatsiyiCmd.Parameters.AddWithValue("@KategoryId", kategoryId);
                                deleteZmistRealizatsiyiCmd.ExecuteNonQuery();
                            }

                            string deleteTovarQuery = "DELETE FROM Товар WHERE ID_категорії = @KategoryId";
                            using (SqlCommand deleteTovarCmd = new SqlCommand(deleteTovarQuery, connection))
                            {
                                deleteTovarCmd.Parameters.AddWithValue("@KategoryId", kategoryId);
                            ...
                            {
                                deleteRealizatsiyaCmd.Parameters.AddWithValue("@KategoryId", kategoryId);
                                deleteRealizatsiyaCmd.ExecuteNonQuery();
                            }
                        }

                        string deleteKategoryQuery = "DELETE FROM Товар_по_категоріям WHERE ID_категорії = @KategoryId";
                        using (SqlCommand deleteKategoryCmd = new SqlCommand(deleteKategoryQuery, connection))
                        {
                            deleteKategoryCmd.Parameters.AddWithValue("@KategoryId", kategoryId);
                            deleteKategoryCmd.ExecuteNonQuery();
                        }
